...
- [ ] **Persona/actor labels are wrapped in a `<rect>` card** — every persona (👤 user, 👩‍🔬 data scientist, 👨‍💻 developer, etc.) must sit inside its own bordered card just like every other diagram element. A bare emoji + text floating in whitespace breaks the visual grammar
- [ ] **Cross-boundary arrows are visually traced after every change** — render the SVG, confirm the path doesn't cross other panels, tiles, labels, or arrows. Prefer orthogonal `<polyline>` paths or smooth beziers that exit one panel's edge and enter the target panel's edge directly. When a feedback arrow would cross a busy region, route it AROUND the busy panel (over the top or down the side), not through it
- [ ] **Cross-boundary arrow labels sit ON TOP of panel content with SOLID white backgrounds** — render labels in the FOREGROUND section after every panel they overlap, and back them with a solid `<rect fill="#fff">` (NO `fill-opacity`) plus a thin matching-color stroke. Semi-transparent label backgrounds let panel borders bleed through and look half-buried. Position labels in the empty gutter between panels whenever possible
- [ ] **Cross-boundary arrows + labels are placed AFTER ALL panels they touch** — not just after the source panel. If the arrow's source is panel A and its label sits in the gutter adjacent to panel B, drawing the arrow right after panel A still lets panel B (drawn later) clip/cover the label. Move the entire arrow + label block to the dedicated FOREGROUND section at the very end of the SVG. Verify by rendering — labels must be fully visible, never clipped by any panel border
- [ ] **Arrow heads are FULLY VISIBLE — never clipped by destination boxes.** Use `markerUnits="userSpaceOnUse"`, `markerWidth="12" markerHeight="12"`, `refX="10"`. Terminate the line 3-5px BEFORE the destination box edge (e.g., end at x=817 if the box left edge is x=820). Lines that end exactly on the edge render the arrow head under the box border, making it invisible
- [ ] **All connector arrows are orthogonal `<polyline>` paths** — NO diagonals through other boxes. Diagonals always end up cutting through some other box or label. Route every cross-region arrow with right-angle bends through gutter/margin space only. Mentally trace each segment's (x,y) range against every other box; if it crosses any, reroute through an empty horizontal band (the gap between rows) or wrap around the edge of the diagram
...
15. **Never leave persona/actor labels as bare text** — user, data scientist, developer, operator, etc. must always be wrapped in a `<rect>` card with stroke + fill, matching the visual treatment of other components. Bare emoji + text floating outside any box looks like an accidental annotation, not a first-class actor in the diagram
16. **Never route cross-boundary arrows through other panels** — a feedback or loop-back arrow that originates in one panel and terminates in another must take a path that stays in empty gutter space between panels. If the direct route crosses other panels' content or other arrows, reroute around (over the top, down the side, or under the bottom of) the obstacle using an orthogonal `<polyline>`. Always render and visually inspect the result — a path that looks plausible in the source XML can still cross labels and tiles in the rendered image
17. **Never use semi-transparent backgrounds (`fill-opacity`) on cross-boundary arrow labels** — the label rect must be SOLID `fill="#fff"` with a thin matching-color stroke (e.g., `stroke="#0ea5e9" stroke-width="0.5"`). Semi-transparent rects let the underlying panel border bleed through and make the label look half-buried inside the panel. Solid rects make the label read clearly as a foreground annotation. Always place these label rects in the FOREGROUND section so they render after every panel they overlap
18. **Never assume "draw after the source panel" puts the arrow on top** — a cross-boundary arrow that starts in panel A and lands near panel B will be CLIPPED by panel B if panel B's `<rect>` is declared later in the SVG. The fix is NOT to nudge the arrow ordering up by one panel; the fix is to move the ENTIRE arrow + label block to the dedicated FOREGROUND section at the end of the SVG (just before the legend). Render and verify the label is fully visible — not even a 1-pixel clip is acceptable
19. **Never use diagonal arrows that cross other boxes.** Diagonals routed corner-to-corner across a diagram inevitably cut through unrelated boxes and labels. Always use orthogonal `<polyline>` routing (right-angle bends only) through gutter/margin space. Out the source edge → along an empty gutter → up/down through an empty column → into the target edge. Mentally trace each segment against every box's bounding box before committing
20. **Never let arrow heads get clipped by the destination box edge.** Arrow markers render at the line's endpoint; if the endpoint is exactly on the box border, the arrow head sits under/inside the border and disappears visually. Always: (a) use `markerUnits="userSpaceOnUse"` with `markerWidth/markerHeight=12` and `refX=10` for clearly visible heads, AND (b) terminate the line 3-5px BEFORE the destination box edge. Render and verify every arrow head is fully visible against the page background, not the destination box fill
